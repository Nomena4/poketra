...
// ==========================================
// CASHPOINT API ENDPOINTS
// ==========================================

// CAISSE
export async function getCaisseTransactions() {
  if (isOfflineMode()) return offlineDb.getCaisseTransactions();
  const res = await fetch(`${API_URL}/caisse`, { headers: getHeaders() });
  if (!res.ok) throw new Error("Erreur de récupération des transactions de caisse");
  return res.json();
}

export async function createCaisseTransaction(data) {
  if (isOfflineMode()) return offlineDb.createCaisseTransaction(data);
  const res = await fetch(`${API_URL}/caisse`, {
    method: "POST",
    headers: getHeaders(),
    body: JSON.stringify(data)
  });
  if (!res.ok) throw new Error("Erreur de création de la transaction");
  return res.json();
}

export async function deleteCaisseTransaction(id) {
  if (isOfflineMode()) return offlineDb.deleteCaisseTransaction(id);
  const res = await fetch(`${API_URL}/caisse/${id}`, {
    method: "DELETE",
    headers: getHeaders()
  });
  if (!res.ok) throw new Error("Erreur de suppression de la transaction");
  return true;
}
